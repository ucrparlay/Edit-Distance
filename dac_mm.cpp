...
template <typename T>
class DAC_MM {
  static constexpr uint32_t VAL_MAX = std::numeric_limits<uint32_t>::max() / 2;
  const sequence<T> &A;
  const sequence<T> &B;
  ...
  sequence<sequence<uint32_t>> merge_horizontal(
    ...
    size_t n = n1 + n2 - k - 1;
    sequence<sequence<uint32_t>> ret(n, sequence<uint32_t>(n, VAL_MAX));
    ...
    parallel_for(0, n1, [&](size_t i) {
      parallel_for(0, n2, [&](size_t j) {
        ret[i][n1 - k - 1 + j] = reduce(
            delayed_seq<uint32_t>(k + 1,
                                  [&](size_t o) {
                                    if (left[i][n1 - k - 1 + o] == VAL_MAX ||
                                        right[o][j] == VAL_MAX) {
                                      return VAL_MAX;
                                    }
                                    return left[i][n1 - k - 1 + o] +
                                           right[o][j];
                                  }),
            minm<uint32_t>());
      });
  ...
  sequence<sequence<uint32_t>> solve_r(size_t i, size_t n, size_t j, size_t m) {
    ...
    if (n == 1 && m == 1) {
      auto ret =
          sequence<sequence<uint32_t>>(3, sequence<uint32_t>(3, VAL_MAX));
      ret[0][0] = ret[2][2] = 0;
      ...
      sequence<sequence<uint32_t>> v1, v2;
      ...
      sequence<sequence<uint32_t>> t1, t2, t3, t4;
      par_do(
          [&]() {
