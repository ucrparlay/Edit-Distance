...
long minimum_edit_distance(const Seq& s1, const Seq& s2) {
  ...
  auto do_block = [&](int* row_in, int* row_out, auto s1, int n, int* col_in,
                      int* col_out, auto s2, int m, int diag) {
    parlay::sequence<int> row(n);
    for (int i = 0; i < n; i++) row[i] = row_in[i];
    for (int j = 0; j < m; j++) {
      int prev = col_in[j];
      for (int i = 0; i < n; i++) {
        prev = ((s1[i] == s2[j]) ? diag : 1 + std::min({diag, row[i], prev}));
        diag = row[i];
        row[i] = prev;
      }
      diag = col_in[j];
  ...
  for (long k = 0; k < nb + mb - 1; k++) {
    ...
    parlay::parallel_for(
        ...
        [&](long j) {
          ...
          do_block(ra.begin() + io, rb.begin() + io, s1.begin() + io,
                   std::min(bsize, n - io), c.begin() + jo, c.begin() + jo,
